...
public class TraceIdPostProcessor implements EnvironmentPostProcessor, ApplicationRunner {
    ...
    private void appendOrWrite(MutablePropertySources propertySources, Map<String, Object> map) {
        ...
        if (propertySources.contains(Constant.PROPERTY_SOURCE_NAME)  && propertySources.get(Constant.PROPERTY_SOURCE_NAME) instanceof MapPropertySource) {
            PropertySource<?> source = propertySources.get(Constant.PROPERTY_SOURCE_NAME);
        ...
        if (target == null) {
            target = new MapPropertySource(Constant.PROPERTY_SOURCE_NAME, map);
        }
        if (!propertySources.contains(Constant.PROPERTY_SOURCE_NAME)) {
